...
def getind(cent, x,y):
    '''
    cent is values in x, y 
    returns the closest index
    '''
    xc, yc = cent
    a = [i for i in range(len(x)) if x[i]>xc]
    b = [i for i in range(len(y)) if y[i]>yc]
    #print(a,b)
    return (a[0],b[0])


def getindV2(cent, x,y):
    '''
    cent is values in x, y 
    returns the closest index
    '''
...
#test's for the above functions.
test_ = 0
if test_:
    a = np.zeros([10,10])
    a[3,3:7] = 2
